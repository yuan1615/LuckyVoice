...
class TextEncoder(nn.Module):
  def __init__(self,
    ...
    # num_param = sum(param.numel() for param in self.parameters())  # 6353664
    # print(num_param)
...
class SynthesizerTrn(nn.Module):
  ...
  def __init__(self, 
    ...
    **kwargs):
    ...
    self.resblock_dilation_sizes = resblock_dilation_sizes    # [[1,3,5], [1,3,5], [1,3,5]],
    ...
    if use_sdp:
      # 是否使用  Stochastic Duration Predictor
  ...
  def forward(self, x, x_lengths, prosody, y, y_lengths, sid=None):
    ...
    # np.save('/home/admin/yuanxin/Bil-vits/attn.npy', neg_cent.cpu().numpy())
    # print(attn.shape)
    # np.save('/home/admin/yuanxin/Bil-vits/test.npy', attn.cpu().numpy())

    w = attn.sum(2)  # [8, 1, 101] 这里代表了每个音素对应的 duration，是根据 Monotonic Alignment Search 得到的
    if self.use_sdp:
      l_length = self.dp(x, x_mask, w, g=g)    # 这里返回的是一个损失
      # print(l_length)
      # print(l_length.shape)
      # raise OSError('end')
    ...
    # print(logs_p.shape)
    # raise OSError('end')
    # 根据 attn 将 text 映射到 谱的长度，Tacotron2中这个是 loaction 的对齐
    z_slice, ids_slice = commons.rand_slice_segments(z, y_lengths, self.segment_size)
    # 这里不是整个音频输入的，8192，约0.3s进行对抗训练

    o = self.dec(z_slice, g=g)
    return o, l_length, attn, ids_slice, x_mask, y_mask, (z, z_p, m_p, logs_p, m_q, logs_q)

  def infer(self, x, x_lengths, prosody, sid=None, noise_scale=1, length_scale=1, noise_scale_w=1., max_len=None):
    x, m_p, logs_p, x_mask = self.enc_p(x, x_lengths, prosody)
    if self.n_speakers > 0:
      g = self.emb_g(sid).unsqueeze(-1) # [b, h, 1]
    else:
